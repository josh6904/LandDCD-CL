...
const DB_KEY = 'dcd_possess_land_v21';
...
window.app = {
    ...
    populateDeptSelects() {
        ...
        // Populate datalist for manual cash
    ...
    addPhaseFromForm(e) {
        ...
        document.getElementById('phase-name').value = ''; // Clear name, keep targets if user wants to repeat? usually reset is safer.
        // e.target.reset(); // Optional: Reset everything
            ...
            lines.forEach((line) => {
                ...
                if (isPersonal) {
                    ...
                    if (onIndex === -1) {
                        ...
                        if(dateFallback) {
                            const dateObj = app.parseSmartMpesaDate(dateFallback[0]);
                            if(dateObj) {
                        ...
                        const dateObj = app.parseSmartMpesaDate(dateStr);
                        if(dateObj) {
                    ...
                    if(tillMatch) {
                         ...
                         const dateObj = dateStr ? app.parseSmartMpesaDate(dateStr) : new Date();
        ...
        this.stagedTx.forEach((tx, idx) => {
            ...
            itemDiv.innerHTML = `
                <div>
                    ...
                    <div style="font-size:0.85rem; color:var(--text-muted);">${tx.method} • ${app.formatDateTime(tx.date)}</div>
                </div>
                <div style="text-align: right;">
                    <div style="font-weight:700; color:var(--primary); margin-bottom: 8px;">${app.formatMoney(tx.amount)}</div>
                    <div>${deptSelect}</div>
                </div>
            `;
    ...
    commitStaged() {
        ...
        this.stagedTx.forEach(tx => {
            ...
            // Simple duplicate check for SMS
            const dupe = store.data.transactions.some(t => {
                const isSamePerson = t.name.toLowerCase() === tx.name.toLowerCase() && 
                                     t.department.toLowerCase() === tx.assignedDept.toLowerCase();
                const isSameAmount = Math.abs(t.amount - tx.amount) < 0.01;
                const txTime = new Date(t.date).getTime();
                const checkTime = tx.date.getTime();
                const isRecent = Math.abs(txTime - checkTime) < 60000; // 1 minute tolerance for SMS
    ...
    renderDashboard() {
        ...
        if(txs.length === 0) {
            ...
            recent.innerHTML = txs.map(t => `
                <div class="activity-item" style="padding: 10px; border-bottom: 1px solid #F3F4F6; display: flex; justify-content: space-between; align-items: center;">
                    <div>
        ...
        // Charts
        MiniCharts.renderDonut('chart-efficiency', rate, '#059669');
            ...
            if(p.name.toLowerCase().includes(search)) {
                ...
                html += `
                    <tr>
                        ...
                        <td><span class="badge" style="background:#F3F4F6; color:#475569;">${escapeHtml(p.department)}</span></td>
...
const MiniCharts = {
    ...
    renderBars(containerId, data) {
        ...
        data.forEach((d, i) => {
            ...
            title.textContent = `${d.label}: ${app.formatMoney(d.value)} / ${app.formatMoney(d.target)}`;
